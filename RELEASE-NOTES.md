# Supergos Release Notes

## v4.0.3 (2025-12-26)

### Improvements

**Strengthened using-supergos skill for explicit skill requests**
...
The underlying skills (`supergos:brainstorming`, `supergos:executing-plans`, `supergos:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-supergos` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-supergos`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `supergos:brainstorming`)
...
**Skill priority in using-supergos**
...
- **Rationalizations table** in using-supergos - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-supergos is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > supergos
...
- Optimized supergos bootstrap to eliminate redundant skill execution. The `using-supergos` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-supergos` manually despite already having the content from session start.
...
- Added unified `supergos-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `supergos:skill-name` for supergos skills, `skill-name` for personal
...
- Updated terminology: "Supergos skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/supergos-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/supergos-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core supergos functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-supergos skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-supergos/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-supergos skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-supergos/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `supergos:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `supergos:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `supergos:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `supergos:code-reviewer`
...
- All internal skill references now use `supergos:` namespace prefix
- Updated format: `supergos:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/supergos:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Supergos v2.0.0 Release Notes

## Overview

Supergos v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([ThePeppy/supergos-skills](https://github.com/ThePeppy/supergos-skills)). This transforms supergos from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [ThePeppy/supergos-skills](https://github.com/ThePeppy/supergos-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/supergos/skills/`
...
1. Your old `~/.config/supergos/.git` will be backed up to `~/.config/supergos/.git.bak`
2. Old skills will be backed up to `~/.config/supergos/skills.bak`
3. Fresh clone of ThePeppy/supergos-skills will be created at `~/.config/supergos/skills/`

### Removed Features

- **Personal supergos overlay system** - Replaced with git branch workflow
- **setup-personal-supergos hook** - Replaced by initialize-skills.sh
...
- Clones ThePeppy/supergos-skills on first run
...
- `SUPERPOWERS_SKILLS_ROOT` set to `~/.config/supergos/skills`
...
- Removed obsolete setup-personal-supergos call from session-start
...
- Prominent link to supergos-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in ThePeppy/supergos-skills
- `scripts/` directory - Now in ThePeppy/supergos-skills/skills/using-skills/
- `hooks/setup-personal-supergos.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/supergos/skills
...
- PR #2: Personal supergos overlay system (later replaced)
...
/plugin install supergos@supergos-marketplace
```
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/supergos/skills ~/supergos-skills-backup
   ```

2. **Update the plugin:**
   ```bash
   /plugin update supergos
   ```
...
- Skills repository is now at https://github.com/ThePeppy/supergos-skills
...
**Full Changelog:** https://github.com/ThePeppy/supergos/compare/dd013f6...main
**Skills Repository:** https://github.com/ThePeppy/supergos-skills
**Issues:** https://github.com/ThePeppy/supergos/issues
