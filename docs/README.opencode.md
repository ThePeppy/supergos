# Supergos for OpenCode

Complete guide for using Supergos with [OpenCode.ai](https://opencode.ai).
...
ln -sf ~/.config/opencode/supergos/.opencode/plugin/supergos.js ~/.config/opencode/plugin/supergos.js
```
...
ln -sf ~/.config/opencode/supergos/.opencode/plugin/supergos.js .opencode/plugin/supergos.js
```

#### 3. Restart OpenCode

Restart OpenCode to load the plugin. Supergos will automatically activate.
...
3. **Supergos skills** (`~/.config/opencode/supergos/skills/`)

You can force resolution to a specific level:
- `project:skill-name` - Force project skill
- `skill-name` - Search project → personal → supergos
- `supergos:skill-name` - Force supergos skill

## Features

### Automatic Context Injection

The plugin automatically injects supergos context via the chat.message hook on every session. No manual configuration needed.
...
The plugin listens for `session.compacted` events and automatically re-injects the core supergos bootstrap to maintain functionality after context compaction.
...
**Location:** `~/.config/opencode/supergos/.opencode/plugin/supergos.js`
...
**Location:** `~/.config/opencode/supergos/lib/skills-core.js`
...
2. Check symlink: `ls -l ~/.config/opencode/plugin/supergos.js`
3. Check OpenCode logs: `opencode run "test" --print-logs --log-level DEBUG`
4. Look for: `service=plugin path=file:///.../supergos.js loading plugin`

### Skills not found

1. Verify skills directory: `ls ~/.config/opencode/supergos/skills`
...
3. Test plugin manually: `node --input-type=module -e "import('file://~/.config/opencode/plugin/supergos.js').then(m => console.log(Object.keys(m)))"`

### Context not injecting

1. Check if chat.message hook is working
2. Verify using-supergos skill exists
