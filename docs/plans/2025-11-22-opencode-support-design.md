...
Add full supergos support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port supergos to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/supergos-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/supergos.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/supergos/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/supergos-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "supergos:brainstorming")')
  }),
...
1. **Inject using-supergos content**
   - Full content of the using-supergos skill
...
export const SupergosPlugin = async ({ client, directory, $ }) => {
  const supergosDir = path.join(process.env.HOME, '.config/opencode/supergos');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSupergos = await readSkill('using-supergos');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingSupergos}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── supergos-codex        # UPDATED: Use skills-core
│   ├── supergos-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── supergos.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/supergos-codex`
...
2. Update `.codex/supergos-codex` to use shared core
...
1. Create `.opencode/plugin/supergos.js`
