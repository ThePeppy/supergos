...
> **For Claude:** REQUIRED SUB-SKILL: Use supergos:executing-plans to implement this plan task-by-task.

**Goal:** Add full supergos support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/supergos-codex` (lines 40-74)
...
- Reference: `.codex/supergos-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'supergos' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/supergos-codex` (lines 212-280)
 ...
 * (personal skills override supergos skills).
 *
 * @param {string} skillName - Name like "supergos:brainstorming" or "my-skill"
 * @param {string} supergosDir - Path to supergos skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, supergosDir, personalDir) {
    // Strip supergos: prefix if present
    ...
    // Try personal skills first (unless explicitly supergos:)
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/supergos-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/supergos-codex` (add import at top)
...
Run: `node -c .codex/supergos-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/supergos-codex
...
**Files:**
- Modify: `.codex/supergos-codex` (lines 40-74)
...
Run: `.codex/supergos-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/supergos-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/supergos-codex` (lines 16-38, approximately)
...
Run: `.codex/supergos-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/supergos.js`
...
/**
 * Supergos plugin for OpenCode.ai
 *
...
const supergosSkillsDir = path.join(homeDir, '.config/opencode/supergos/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SupergosPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/supergos.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/supergos.js
...
**Files:**
- Modify: `.opencode/plugin/supergos.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "supergos:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > supergos)
...
Run: `node -c .opencode/plugin/supergos.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the supergos and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const supergosSkills = skillsCore.findSkillsInDir(
            supergosSkillsDir,
            'supergos',
            3
          );
          ...
          const allSkills = [...personalSkills, ...supergosSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install supergos skills to ~/.config/opencode/supergos/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'supergos:';
...
```javascript
    'session.started': async () => {
      // Read using-supergos skill content
      const usingSupergosPath = skillsCore.resolveSkillPath(
        'using-supergos',
        supergosSkillsDir,
        personalSkillsDir
      );

      let usingSupergosContent = '';
      if (usingSupergosPath) {
        const fullContent = fs.readFileSync(usingSupergosPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSupergosContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Supergos skills: \`supergos:skill-name\` (from ~/.config/opencode/supergos/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/supergos')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/supergos && git pull` to update supergos.' :
        '';
...
**Below is the full content of your 'supergos:using-supergos' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingSupergosContent}
...
```markdown
# Installing Supergos for OpenCode
...
The plugin is included in the supergos repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/supergos/.opencode/plugin/supergos.js`
...
ln -s ~/.config/opencode/supergos/.opencode/plugin/supergos.js .opencode/plugin/supergos.js
```
...
```
use use_skill tool with skill_name: "supergos:brainstorming"
```
...
Personal skills override supergos skills with the same name.

## Updating

```bash
cd ~/.config/opencode/supergos
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/supergos/.opencode/plugin/supergos.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/supergos/skills`
...
Supergos works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/supergos-codex use-skill supergos:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/supergos-codex bootstrap | head -30`
...
ls -l .opencode/plugin/supergos.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/supergos.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/supergos-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/supergos-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/supergos.js` created with tools and hooks
