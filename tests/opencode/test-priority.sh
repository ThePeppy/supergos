...
# Verifies that skills are resolved with correct priority: project > personal > supergos
...
# 1. Create in supergos location (lowest priority)
mkdir -p "$HOME/.config/opencode/supergos/skills/priority-test"
cat > "$HOME/.config/opencode/supergos/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Supergos version of priority test skill
---
# Priority Test Skill (Supergos Version)
...
if [ -f "$HOME/.config/opencode/supergos/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Supergos version exists"
else
    echo "  [FAIL] Supergos version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides supergos
echo ""
echo "Test 2: Testing personal > supergos priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides supergos)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [FAIL] Supergos version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|supergos" | head -10
fi

# Test 3: Test that project overrides both personal and supergos
echo ""
echo "Test 3: Testing project > personal > supergos priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Supergos version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit supergos: prefix bypasses priority
echo ""
echo "Test 4: Testing supergos: prefix forces supergos version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load supergos:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] supergos: prefix correctly forces supergos version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] supergos: prefix did not force supergos version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
